...
export function FaqList({ items }: { items: FaqItem[] }) {
  return (
    <div className="space-y-4">
      {items.map((item) => (
        <details key={item.question} className="site-feature-card rounded-3xl border border-[color:var(--border)] bg-white/80 p-5 dark:bg-slate-800/90 dark:border-slate-700/60">
